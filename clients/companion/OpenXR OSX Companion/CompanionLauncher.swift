...
enum RuntimeInstallError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingBundledRuntime:
            return "This build does not include a bundled OpenXR runtime."
...
struct CompanionRuntimeInstaller {
    ...
    init(
        bundledRuntimeDirectory: URL? = CompanionPaths.bundledRuntimeDirectoryURL,
        installedRuntimeDirectory: URL = URL(fileURLWithPath: CompanionPaths.installedRuntimeDirectory, isDirectory: true),
        userConfigFile: URL = URL(fileURLWithPath: CompanionPaths.configFilePath),
    ...
    var installedManifestURL: URL {
        installedRuntimeDirectory.appendingPathComponent("openxr_osx.json")
    }

    var installedLibraryURL: URL {
        installedRuntimeDirectory.appendingPathComponent("libopenxr_osx.dylib")
    }

    func status() -> RuntimeInstallStatus {
        let bundledLibraryURL = bundledRuntimeDirectory?.appendingPathComponent("libopenxr_osx.dylib")
    ...
    func installBundledRuntime() throws -> String {
        ...
        let bundledConfigURL = bundledRuntimeDirectory.appendingPathComponent("openxr_osx.toml")
        let installedConfigURL = installedRuntimeDirectory.appendingPathComponent("openxr_osx.toml")
        if fileManager.fileExists(atPath: bundledConfigURL.path) {
            ...
            try OpenXRServerConfig.defaultText.write(to: userConfigFile, atomically: true, encoding: .utf8)
        }
