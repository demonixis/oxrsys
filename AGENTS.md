...
- The embedded-ALVR path selects its codec separately (`SelectAlvrVideoCodec`): ALVR v20.14.1 clients decode H.264/HEVC unconditionally, the native encoder helper (`streaming.encoder_process`) lifts the Rosetta H.264 restriction, and the selected codec must stay in sync with ALVR's `session.json` `video.preferred_codec` (written by `SyncSessionSettings` before `alvr_initialize`). Encoder rebuilds key on the full generation identity (`AlvrEncoderSelection.h`); a reconnect never inherits the previous encoder, and helper crash fallback follows the 30s-respawn/two-failure-pin budget with `encoder_process = "native"` never silently pinning.
- The encoder helper build is thin-arm64-only: `OXRSYS_BUILD_ENCODER_HELPER=ON` on any configure that is not exactly single-arch `arm64` is a configure-time FATAL_ERROR, and the non-arm64 configure sweep must spare a staged arm64 helper binary in `runtime/` (packaging scripts stage the native helper into x86_64 trees) while still removing anything else — see `docs/build.md`.
...
- Apple VideoToolbox streams use a BT.709 SDR, limited-range YCbCr color contract. The encoder feeds BGRA and VT performs the RGB→YCbCr conversion internally — the contract is declared through session properties and per-buffer color attachments (primaries/transfer/matrix all ITU-R 709), and those must stay aligned with client conversion, including exact normalized code ranges for 8-bit and 10-bit bi-planar decoder surfaces. Under Rosetta this requires macOS 27+ (older VT emits all-zero chroma for BGRA input; verified via wine-vr's `vt-llrc-probe --matrix`).
...
- The TOML-subset config parser strips same-line `#` comments outside quoted values (no escape handling exists; `"` toggles string context). This is a behavioral contract with regression tests in `TestConfig.cpp` — keep comment handling working for string, numeric, and bool keys.
- Tests must never write the real user config: run suites through `ctest` (isolated `HOME` under the build tree); direct test-binary runs overwrite `~/Library/Application Support/OXRSys/oxrsys-runtime.toml` — see `docs/testing-and-conformance.md`.
