...
- Added the out-of-process native encoder groundwork on macOS: a new `oxrsys-encoder-helper` executable (built by `OXRSYS_BUILD_ENCODER_HELPER` — see the build-contract note at the end of the next entry) hosts the shared VideoToolbox encode engine in a native-arm64 process, and a `NativeHelperEncoderTransport` implements the encoder transport seam against it — versioned little-endian socket protocol, bootstrap-namespace Mach rendezvous with IOSurface send-right transfer (wrap-once + BT.709 re-attachment), trial-create capability handshake that rejects translated children, bounded non-blocking frame submission, and crash-safe exactly-once frame resolution.
- Wired the native encoder helper into the ALVR streaming backend: a new `streaming.encoder_process = "auto" | "native" | "inproc"` config key selects the encode process (default `auto` uses the helper when its binary sits next to the runtime dylib, else falls back in-process; `native` fails loudly instead of degrading). With the helper active, codec selection honors `video_codec` even under Rosetta (hardware HEVC out-of-process; `auto` prefers H.265), and the selected codec is synced into ALVR's `session.json` `video.preferred_codec` before `alvr_initialize` so the client is announced exactly what the encoder produces; without the helper the in-process Rosetta H.264 gate is unchanged. The encoder rebuild decision now keys on the full generation identity (resolution, fps, codec, bit depth, pixel format, transport mode) and a client reconnect always re-runs selection and re-sends config NALs instead of inheriting the previous encoder. Helper crashes drop in-flight frames, re-arm a keyframe, and rebuild under a respawn budget: at most one automatic respawn per 30 s, and after two failures within one connected session `auto` pins to in-process H.264 until the next reconnect (`native` never silently pins). Build contract: `OXRSYS_BUILD_ENCODER_HELPER` defaults ON only for thin-arm64 configures, and enabling it on any other configure (x86_64, arm64e, universal) is a configure-time error — the helper is built in its own arm64 tree and staged next to the runtime dylib by packaging scripts, and a non-arm64 tree must never build over it (see `docs/build.md`).
...
- Extracted the VideoToolbox-specific half of the macOS encoder into `runtime/src/encoder/VideoToolboxEncodeEngine.{h,mm}` behind a new minimal transport seam (`runtime/src/encoder/EncoderTransport.h`, implemented in-process by `runtime/src/encoder/InProcessEncoderTransport.{h,mm}`), as preparation for the planned native-arm64 out-of-process encoder helper; `VideoEncoder` keeps the Metal compose path (slots, shared-event waits, crop/MPS/blit) and drives the engine through the seam with unchanged behavior, properties, callbacks, and drain/shutdown ordering. Two latent output-path bugs were fixed while extracting: the sample NAL-unit walk now honors the AVCC/HVCC NAL-unit length-prefix size reported by the format description instead of assuming 4 bytes, and keyframe parameter sets are now classified by NAL type and emitted in VPS/SPS/PPS order instead of trusting VideoToolbox's index order.
- The VideoToolbox encoder feeds the composed BGRA frame to VT directly and lets VT do the RGB→YCbCr conversion internally; the `rgb_to_nv12` Metal pre-convert kernel, the NV12 pixel-buffer pool, and the per-slot Y/CbCr plane textures are removed, and the BGRA compose target is now a view of the encoder's own IOSurface-backed pixel buffer (one less compute dispatch per frame). Under Rosetta this requires macOS 27+ — Apple fixed VT's zero-chroma BGRA bug there, and the BT.709 limited-range match is verified numerically by wine-vr's `vt-llrc-probe --matrix`; on older macOS the encoder logs a warning and video will be green. Known accepted trade-off (live A/B on a Quest 3, 3008x1664@72): VT's internal conversion of GPU-written IOSurfaces is slow under Rosetta (~25 ms inside VT vs ~2 ms for the old GPU kernel), raising encode latency from ~13-22 ms to ~35-40 ms and client-measured motion-to-photon from ~82-96 ms to ~114 ms. The native-arm64 out-of-process encoder helper (added in this release, see above) is the path that recovers this and unlocks HW HEVC; the trade-off now applies only to the in-process `encoder_process = "inproc"` fallback.
- Client keyframe requests are now rate-limited where they arrive (the wire keyframe handler and the ALVR IDR event) instead of inside `VideoEncoder::ForceKeyframe`, which is unconditional again; internal connect-warmup, GOP, and reconfigure forces are naturally exempt. Suppressed requests still count toward ABR distress signals.
- The encoder no longer falls back to a non-low-latency VideoToolbox session when the low-latency create fails; it fails loudly instead. The fallback never fired in any recorded run, and a silent non-LL session would degrade latency behavior unnoticed.
- Merged upstream develop `86bde247` into this fork, bringing codec capability negotiation, the per-backend swapchain source split, the low-latency client decode path, and the v1.3.0 feature set. (The "Wine/Rosetta encodes H.264 only regardless of `video_codec`" statement that originally accompanied this merge is superseded within this release: with the native encoder helper active, `video_codec` is honored under Rosetta — hardware HEVC out-of-process. The H.264-only gate now applies only to the in-process `encoder_process = "inproc"` path.)
...
- `encoder_process = "auto"` now warns once per connected session when it falls back to the in-process encoder because the helper binary is missing (previously the only trace was the generic `encoder ready … (H.264, in-process)` line, which reads as normal steady state), and the session.json sync line states why the helper would not run (`encoder_process=inproc`, helper absent, or helper absent with Rosetta forcing H.264).
- Fixed the runtime CMake configure deleting a deliberately staged native-arm64 `oxrsys-encoder-helper` from a non-arm64 tree: the stale-helper sweep now removes only a binary that is not arm64 (word-boundary `lipo` match; unreadable files still removed), so re-configuring an x86_64 tree no longer un-stages the helper. Configuring any tree that is not thin-arm64 with `OXRSYS_BUILD_ENCODER_HELPER=ON` is now a configure-time error instead of silently building a wrong-arch helper over the staged one; in thin-arm64 trees a leftover helper is still swept unconditionally (it is that tree's own stale output).
...
- A client display standby without a stream disconnect (observed with the stock ALVR Quest client, 2026-08-04) starves the frame loop — the session stays FOCUSED and tracking/battery keep flowing while encode stops, so no reconnect path fires; the frozen frame recovers only on full headset wake. No server-side fix is planned yet; client-side behavior is under investigation.
- Running test binaries directly (outside `ctest`) overwrites the REAL user config at `~/Library/Application Support/OXRSys/oxrsys-runtime.toml`; `ctest` isolates `HOME` under the build tree — see `docs/testing-and-conformance.md`.
