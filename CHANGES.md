...
- Added the out-of-process native encoder groundwork on macOS: a new `oxrsys-encoder-helper` executable (built by `OXRSYS_BUILD_ENCODER_HELPER`, default ON on Apple) hosts the shared VideoToolbox encode engine in a native-arm64 process, and a `NativeHelperEncoderTransport` implements the encoder transport seam against it — versioned little-endian socket protocol, bootstrap-namespace Mach rendezvous with IOSurface send-right transfer (wrap-once + BT.709 re-attachment), trial-create capability handshake that rejects translated children, bounded non-blocking frame submission, and crash-safe exactly-once frame resolution. The streaming backend still uses the in-process transport; switching is follow-up work.
