...
public:
    ...
    void SetGraphicsDevice(GraphicsApi graphicsApi, void* graphicsDevice)
    {
        ...
        graphicsContext.api = graphicsApi;
        if (graphicsApi == GraphicsApi::Metal)
        {
            graphicsContext = GraphicsContext::Metal(graphicsDevice);
        }
#ifdef XR_USE_GRAPHICS_API_VULKAN
        else if (graphicsApi == GraphicsApi::Vulkan && graphicsDevice != nullptr)
        {
            graphicsContext =
                GraphicsContext::Vulkan(*static_cast<const VulkanGraphicsContext*>(graphicsDevice));
        }
#endif
#if defined(_WIN32)
        else if (graphicsApi == GraphicsApi::D3D11 && graphicsDevice != nullptr)
        {
            graphicsContext = GraphicsContext::D3D11(
                *static_cast<const D3D11GraphicsContext*>(graphicsDevice));
        }
        else if (graphicsApi == GraphicsApi::D3D12 && graphicsDevice != nullptr)
        {
            graphicsContext = GraphicsContext::D3D12(
                *static_cast<const D3D12GraphicsContext*>(graphicsDevice));
        }
#endif
